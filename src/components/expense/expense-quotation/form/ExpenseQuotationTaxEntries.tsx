...
import { ArticleExpenseQuotationEntry, Currency, Tax } from '@/types';
...
export const ExpenseQuotationTaxEntries: React.FC<ExpenseQuotationTaxEntriesProps> = ({
  className,
  article,
  taxes,
  selectedTaxIds,
  ...
  return (
    <div className={cn('flex flex-col', className)}>
      {(!article?.articleExpenseQuotationEntryTaxes ||
        article?.articleExpenseQuotationEntryTaxes?.length == 0) && (
        <p className="font-thin text-sm m-1">{tInvoicing('article.no_applied_tax')}</p>
      )}
      {article.articleExpenseQuotationEntryTaxes?.map((appliedTax, i) => (
