...
import { ArticleExpenseQuotationEntry, Currency, ExpenseQuotationTaxEntry, Tax } from '@/types';
import {
  Select,
  SelectTrigger,
  SelectContent,
...
interface ExpenseQuotationArticleItemProps {
  className?: string;
  article: ArticleExpenseQuotationEntry;
  onChange: (item: ArticleExpenseQuotationEntry) => void;
...
export const ExpenseQuotationArticleItem: React.FC<ExpenseQuotationArticleItemProps> = ({
  ...
  const handleTaxChange = (value: string, index: number) => {
    ...
    const updatedTaxes = [...(article.articleExpenseQuotationEntryTaxes || [])];
    if (selectedTax) {
      updatedTaxes[index] = { tax: selectedTax };
    } else {
      updatedTaxes.splice(index, 1);
    }
    onChange({ ...article, articleExpenseQuotationEntryTaxes: updatedTaxes });
  };

  const handleTaxDelete = (index: number) => {
    const updatedTaxes = article.articleExpenseQuotationEntryTaxes?.filter((_, i) => i !== index);
    onChange({ ...article, articleExpenseQuotationEntryTaxes: updatedTaxes });
  };
  ...
  const handleAddTax = () => {
    ...
    onChange({
      ...
      articleExpenseQuotationEntryTaxes: [
        ...(article.articleExpenseQuotationEntryTaxes || []),
        {} as ExpenseQuotationTaxEntry
      ]
    });
  };

  const selectedTaxIds = article.articleExpenseQuotationEntryTaxes?.map((t) => t.tax?.id) || [];
                ...
                article.article?.description && (
                  <>
                    ...
                    <Textarea
                      ...
                      className="resize-none"
                      onClick={() => {}}
                      rows={3 + (article?.articleExpenseQuotationEntryTaxes?.length || 0)}
                    />
                  </>
                )
              )}
