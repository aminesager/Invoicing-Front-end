import { ArticleExpenseQuotationEntry, Tax } from '@/types';
import { DISCOUNT_TYPE } from '@/types/enums/discount-types';
import { v4 as uuidv4 } from 'uuid';
import { create } from 'zustand';

type ExpenseQuotationPseudoItem = {
  id: string;
  article: ArticleExpenseQuotationEntry & { total?: number };
};

export type ExpenseQuotationArticleManager = {
  articles: ExpenseQuotationPseudoItem[];
  taxSummary: { tax: Tax; amount: number }[];
  add: (article?: ArticleExpenseQuotationEntry) => void;
  update: (id: string, article: ArticleExpenseQuotationEntry) => void;
  delete: (id: string) => void;
  setArticles: (articles: ArticleExpenseQuotationEntry[]) => void;
  reset: () => void;
  getArticles: () => (ArticleExpenseQuotationEntry & { total: number })[];
  //feature
  removeArticleDescription: () => void;
};

const calculateForExpenseQuotation = (article: ArticleExpenseQuotationEntry) => {
  ...
  if (article?.articleExpenseQuotationEntryTaxes) {
    for (const entry of article?.articleExpenseQuotationEntryTaxes) {
      if (entry?.tax?.isRate) {
...
const calculateTaxSummary = (articles: ExpenseQuotationPseudoItem[]) => {
  ...
  articles.forEach((articleItem) => {
    ...
    const taxes = article.articleExpenseQuotationEntryTaxes || [];
...
export const useExpenseQuotationArticleManager = create<ExpenseQuotationArticleManager>()(
  (set, get) => ({
    articles: [],
    taxSummary: [],

    add: (article: ArticleExpenseQuotationEntry = {}) => {
    ...
    update: (id: string, article: ArticleExpenseQuotationEntry) => {
      const { subTotal, total } = calculateForExpenseQuotation(article);
    ...
    setArticles: (articles: ArticleExpenseQuotationEntry[]) => {
      set({
