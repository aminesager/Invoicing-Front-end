import { ArticleExpenseInvoiceEntry, Tax } from '@/types';
import { DISCOUNT_TYPE } from '@/types/enums/discount-types';
import { v4 as uuidv4 } from 'uuid';
import { create } from 'zustand';

type ExpenseInvoicePseudoItem = {
  id: string;
  article: ArticleExpenseInvoiceEntry & { total?: number };
};

export type ExpenseInvoiceArticleManager = {
  articles: ExpenseInvoicePseudoItem[];
  taxSummary: { tax: Tax; amount: number }[];
  add: (article?: ArticleExpenseInvoiceEntry) => void;
  update: (id: string, article: ArticleExpenseInvoiceEntry) => void;
  delete: (id: string) => void;
  setArticles: (articles: ArticleExpenseInvoiceEntry[]) => void;
  reset: () => void;
  getArticles: () => (ArticleExpenseInvoiceEntry & { total: number })[];
  //feature
  removeArticleDescription: () => void;
};

const calculateForExpenseInvoice = (article: ArticleExpenseInvoiceEntry) => {
  ...
  if (article?.articleExpenseInvoiceEntryTaxes) {
    for (const entry of article?.articleExpenseInvoiceEntryTaxes) {
      if (entry?.tax?.isRate) {
...
const calculateTaxSummary = (articles: ExpenseInvoicePseudoItem[]) => {
  ...
  articles.forEach((articleItem) => {
    ...
    const taxes = article.articleExpenseInvoiceEntryTaxes || [];
...
export const useExpenseInvoiceArticleManager = create<ExpenseInvoiceArticleManager>()(
  (set, get) => ({
    articles: [],
    taxSummary: [],

    add: (article: ArticleExpenseInvoiceEntry = {}) => {
      const { subTotal, total } = calculateForExpenseInvoice(article);
    ...
    update: (id: string, article: ArticleExpenseInvoiceEntry) => {
      const { subTotal, total } = calculateForExpenseInvoice(article);
    ...
    setArticles: (articles: ArticleExpenseInvoiceEntry[]) => {
      set({
    ...
    getArticles: () => {
      return get().articles.map((item) => {
        const { subTotal, total } = calculateForExpenseInvoice(item.article);
