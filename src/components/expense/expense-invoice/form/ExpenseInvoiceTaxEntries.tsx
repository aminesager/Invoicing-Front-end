...
import { ArticleExpenseInvoiceEntry, Currency, Tax } from '@/types';
  ...
  return (
    <div className={cn('flex flex-col', className)}>
      {(!article?.articleExpenseInvoiceEntryTaxes ||
        article?.articleExpenseInvoiceEntryTaxes?.length == 0) && (
        <p className="font-thin text-sm m-1">{tInvoicing('article.no_applied_tax')}</p>
      )}
      {article.articleExpenseInvoiceEntryTaxes?.map((appliedTax, i) => (
